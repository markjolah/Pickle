...
classdef Pickle < matlab.mixin.Copyable
    % The Pickle class allows a an object to use methods save() to save itself to a file aand load() to load
    % from a file with a specific extension.
    %
    % Pickle It has a notion of a obj.workingDir which is the "home" directory for the object, where
    % its save file will reside.  The obj.Paths structure is a list of other related files and their
    % relative paths.
    %
    % A pickle object is able to have a blank uninitialized state.  It can be reset to this state with
    ...
    %
    % Notes:
    %  * This class uses the metaclass.PropertyList property to get the definitive list of a class's
    ...
    properties (Transient=true)
        workingDir; %This is wherever we are currently working from.
                    %This is not saved as a permanent property because we can
    ...
    methods
        function params = getParamStruct(obj)
            %
            % This returns a structure version of the class with all the important permanent properties
            % saved.  This same structure is used internally for the save/load format and for the
            % preservation of properties and the use of default properties.
            %
        ...
        function save(obj)
            ...
            obj.updateFromPreservedProperties(); %Fill in any blank (unset) parameters with the preserved parameters.
        ...
        function resetObject(obj)
            ...
            % reset all permanent and transient properties of this object to the default.  Clears any
            % preserved properties.
            % Overload this function in a subclass to allow preservation of some variable (e.g. gui
            % stuff that is independent of object state)
            mc = metaclass(obj);
    ...
    methods (Abstract = true, Access = protected)
        ...
        % subclass.  Unfortunately we cannot directly modify the protected
    ...
    methods (Access=protected)
        ...
        function propnames = getPermenantPropertyNames(obj)
            ...
            propnames = cellfun(@(p) p.Name,props,'uniform',0);
        end
        ...
        function propstruct = getPermenantProperties(obj)
            ...
            propvals = cellfun(@(p) obj.getProtectedProperty(p), props,'uniform',0);
...
%             end
        ...
        function reloadobj(obj,propstruct)
            if isfield(propstruct,'Paths')
                %Check saved file paths use the appropriate file path
                %slashes
    ...
    methods (Static=true)
        ...
        function filename = findUnusedFileName(path,filepattern, idx)
            % Find an used filename with a given pattern which includes a
            % '%i' field for an integer index.
            % [IN]
            %  path - type:string - where the file will live
            %  pattern - type:string - The desired file name with extension.  If it contains a '%i' sub-string then we
            %             will use sprintf with the 'idx' variable which defaults to 1 to substitute in
            ...
            %             idx will be substituted in until an unused name is found.
            %  idx - [optional] type:integer - the starting index for substitution into the file name pattern
            %        [default=1]
            % [OUT]
            %   filename - type:string - full path name to the first unused file name
        ...
        function filepath = selectUnusedFileName(path,filepattern,formats,title)
            % [IN]
            %  path - the full path to the directory.  Will be created if non-existent
            %  filepattern - A suggested filename with extensions with an optional %i substitution as in
        ...
        function filepaths = listExistingFileNames(path,filepattern)
            % [IN]
            %  path - A full path to a directory.  If non exist ant no files will be returned.
            %  filepattern - A pattern with '*' and '?' characters as interpreted by the 'dir' function
            % [OUT]
            ...
            elseif ~exist(path,'dir')
                ...
                filepaths = cellfun(@(n) fullfile(path,n), {filelist(:).name},'uniform',0);
            end
        end
        ...
        function filepath = selectExistingFileName(path,filepattern,formats, title, varargin)
            % [IN]
            %  path - the full path to the directory.  Will be created if non-existent
            %  filepattern - A suggested filename with extensions with * or ? characters
            %  formats - The allowable formats in uiputfile format.
            %  title [optional] - The title of the uiputfile window.
            % [OUT]
            ...
            if isscalar(file) && ~file
                ...
                files = makecell(file);
                filepath = cellfun(@(f) fullfile(path,f), files,'uniform',0);
            end
        ...
        function filepaths = selectBatchProccesingFileNames(path,filepattern,formats, title)
            ...
            if ischar(filepattern) % Add a '*' if only an extension was given
            ...
            if ~isempty(names)
                ...
                elseif ischar(files)
                    ...
                    filepaths = cellfun(@(f) fullfile(path,f), files,'uniform',0);
                end
            end
        end
        function newfs = changeFileExtensions(fs, new_ext)
            ...
            newfs = cellfun(@changeExt, fs,'uniform',0);
        end
        
        function overwrite = confirmOverwriteDialog(fileList)
            % Make a dialog to ask user if it is okay to batch overwrite fails.
            % [in]
